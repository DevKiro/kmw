(() => {
	...
	var PagePagination = class extends CustomHTMLElement {
		...
		_onLinkClicked(event, target) {
			...
			// Get the scroll offset from the CSS variable, with a fallback to 0
			let scrollOffset = getComputedStyle(document.documentElement).getPropertyValue("--scroll-offset").trim();
			scrollOffset = scrollOffset ? parseInt(scrollOffset, 10) : 0;

			// Scroll to the top of the page with smooth behavior and the offset
			window.scrollTo({ top: -scrollOffset, behavior: "smooth" });

			const url = new URL(window.location.href);
			url.searchParams.set("page", target.getAttribute("data-page"));

			// Trigger the custom event, which will presumably start the AJAX request
	...
	// js/custom-element/behavior/sticky-and-sticky-scroll-logic.js
	(function () {
		"use strict";
			...
			elements: [
				{
					...
					toggleClassName: "scrolled-past-top-announcement",
					behavior: "pastTop",
					cssVar: "--scroll-past-top-announcement",
				},
				{
					className: "header",
					toggleClassName: "scrolled-past-top-header",
					behavior: "pastTop",
					cssVar: "--scroll-past-top-header",
				},
				{
					className: "header",
					toggleClassName: "scrolled-past-header",
					behavior: "pastElement",
					cssVar: "--scroll-past-header",
				},
			],
			stickyClasses: [
				{
					cssVar: "--enable-sticky-header",
					className: "has-sticky-header",
				},
				{
					cssVar: "--enable-sticky-announcement-bar",
					className: "has-sticky-announcement-bar",
				},
			],
		};

		// Variable to track if 'page-scrolling' class has been added
		let hasPageScrollingClassBeenAdded = false;
		...
		function initScrollBehavior() {
			...
			function getClientHeight(el) {
				if (elementCache[el.className] === undefined) {
					elementCache[el.className] = el.clientHeight;
				}
				return elementCache[el.className];
			}

			const elements = scrollConfig.elements
				.map(({ className, ...rest }) => {
					const el = document.querySelector(`.${className}`);
					if (!el) console.warn(`Element with class ${className} not found.`);
					return el ? { el, ...rest } : null;
				})
				.filter(Boolean);

			function updateElement({ el, toggleClassName, behavior, cssVar }) {
				const scrollY = window.scrollY;
				const clientHeight = getClientHeight(el);
				const threshold = behavior === "pastElement" ? clientHeight : 0;

				if (scrollY > threshold) {
					htmlElement.classList.add(toggleClassName);
				} else {
					htmlElement.classList.remove(toggleClassName);
				}

				if (cssVar) {
					let pixelsPastTop = Math.min(scrollY - threshold, clientHeight);
					pixelsPastTop = Math.max(pixelsPastTop, 0);
					htmlElement.style.setProperty(cssVar, pixelsPastTop === 0 ? ZERO_PX : pixelsPastTop + "px");
				}
			}
			...
			function onScroll() {
				requestAnimationFrame(() => {
					...
					if (!hasPageScrollingClassBeenAdded && window.scrollY > 0) {
						htmlElement.classList.add("page-scrolling");
						hasPageScrollingClassBeenAdded = true;
					} else if (hasPageScrollingClassBeenAdded && window.scrollY === 0) {
						htmlElement.classList.remove("page-scrolling");
						hasPageScrollingClassBeenAdded = false;
					}
				});
			}
		...
		// New function to apply sticky classes based on CSS variables
		function applyStickyClasses() {
			const htmlElement = document.documentElement;
			const style = getComputedStyle(htmlElement);

			scrollConfig.stickyClasses.forEach(({ cssVar, className }) => {
				const enableFeature = style.getPropertyValue(cssVar).trim();
				if (enableFeature === "1") {
...
`;
			...
			return new Promise((resolve) => {
				requestAnimationFrame(() => {
					...
					this.innerHTML = Object.values(bounds)
						.map((item) => `<span ${this.hasAttribute("reveal") && !force ? "reveal" : ""} ${this.hasAttribute("reveal-visibility") && !force ? "reveal-visibility" : ""} style="display: block">${item.trim()}</span>`)
						.join("");
	...
	// js/custom-element/section/media-with-text-overlays/media-with-text-overlays.js
	var MediaWithOverlays = class extends CustomHTMLElement {
		connectedCallback() {
		...
		async _setupVisibility() {
			await this.untilVisible();
			const image = this.querySelector(".media-overlay__media"),
			...
			const innerEffect = [
				...
				new CustomKeyframeEffect(this.querySelector(".media-overlay__text-container"), { opacity: [0, 1] }, { duration: 300 }),
			];
		...
		_onScroll() {
			...
			requestAnimationFrame(() => {
				const boundingRect = this.getBoundingClientRect(),
					speedFactor = 3,
					contentElement = this.querySelector(".media-overlay__content-wrapper"),
					imageElement = this.querySelector(".media-overlay__media"),
	...
	window.customElements.define("media-with-overlays", MediaWithOverlays);
	...
	var ProductFacet = class extends CustomHTMLElement {
		...
		async _rerender(event) {
			...
			try {
				...
				const scrollTo = this.querySelector(".product-facet__meta-bar") || this.querySelector(".product-list") || this.querySelector(".product-facet__main");
				requestAnimationFrame(() => {
	...
	var FacetFilters = class extends DrawerContent {
		...
		_adjustDrawer(match) {
			...
			this.classList.toggle("drawer--from-right", match.matches);
		}
	};
